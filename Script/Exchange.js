...
$.http.get({url: "https://api.ratesapi.io/api/latest?base=HKD"})
    .then((response) => {
        ...
        const info = Object.keys(currencyNames).reduce((accumulator, key) => {
            ...
            if (key !== base && data.rates.hasOwnProperty(key)) {
                ...
                if (rate > 1) {
                    line = `${target[1]} 1${source[0]} to ${roundNumber(rate, digits)}${
                        target[0]
                    }\n`;
                } else {
                    line = `${target[1]} 1${target[0]} to ${roundNumber(1 / rate, digits)}${
                        source[0]
                    }\n`;
                }
            }
        ...
        $.notify(
            `Today's Exchange Rate Base on ${source[1]} ${source[0]}`,
