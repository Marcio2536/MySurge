...
const carrierNames = {
  //香港電信業者
    '454-00': 'CSL', '454-02': 'CSL', '454-10': 'Sun Mobile', '454-18': 'CSL',
   ...
   '450-06': 'LG U+', '450-10': 'LG U+','467-193': 'Sun Net',
   //美國電信業者
...
if (!v4.primaryAddress && !v6.primaryAddress) {
  ...
  $httpClient.get('http://ip-api.com/json', function (error, response, data) {
    if (error) {
      $done({
        title: 'Error',
        content: 'Could not get current network information\nPlease check your network status',
        icon: 'wifi.exclamationmark',
        'icon-color': '#3478F5',
      });
    }
    ...
    $done({
      ...
      content:
        (v4.primaryAddress ? `Internal IP : ${v4.primaryAddress} \n` : '') +
        (v6.primaryAddress ? `IPv6 : ${v6.primaryAddress}\n` : '') +
        (v4.primaryRouter && wifi.ssid ? `Router IP : ${v4.primaryRouter}\n` : '') +
        (v6.primaryRouter && wifi.ssid ? `Router IPv6 : ${v6.primaryRouter}\n` : '') +
        `External IP : ${info.query}\n` +
        `ISP : ${info.isp}\n` +
        `IP Location : ${getFlagEmoji(info.countryCode)} | ${info.country} - ${info.city
        }`,
      icon: wifi.ssid ? 'wifi' : 'simcard',
      'icon-color': wifi.ssid ? '#3478F5' : '#F9BF45',
    });
  });
}
